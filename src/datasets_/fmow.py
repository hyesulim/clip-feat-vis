...
class FMOW:
    ...
    def __init__(self,
        ...
        self.dataset = FMoWDataset(root_dir=location)
    ...
    def post_loop_metrics(self, labels, preds, metadata, args):
        ...
        hard_preds = preds.argmax(dim=1, keepdim=True).view_as(labels)
        results = self.dataset.eval(hard_preds, labels, metadata, pred_soft=preds)
        dict_acc, dict_ece = results[0], results[2]
        dict_acc.update(dict_ece)
        return dict_acc
...
class ECE(Metric):
    def __init__(self, prediction_fn=None, name=None):
        ...
        self.ece = MulticlassCalibrationError(num_classes=62, n_bins=10, norm='l1')

    def _compute(self, y_pred, y_true):
        if self.prediction_fn is not None:
            y_pred = self.prediction_fn(y_pred)
        return self.ece(y_pred, y_true)
...
class FMoWDataset(WILDSDataset):
    ...
    def __init__(self, version=None, root_dir='data', download=False, split_scheme='official', seed=111, use_ood_val=True):
        ...
        if self._split_scheme.startswith('time_after'):
            ...
            year_minus_3_dt = datetime.datetime(year-3, 1, 1)
        ...
        for year in range(2002, 2018):
            ...
            year_mask = np.asarray(ts >= datetime.datetime(year, 1, 1)) \
                        & np.asarray(ts < datetime.datetime(year+1, 1, 1))
    ...
    def eval(self, y_pred, y_true, metadata, pred_soft, prediction_fn=None):
        #! need to be modifed
        """
        ...
        metric_cal = ECE(prediction_fn=prediction_fn)
        ...
        all_results_ece, all_results_str_ece = self.standard_group_eval(
            metric_cal,
            self._eval_groupers['year'],
            pred_soft, y_true, metadata)
        ...
        region_results_ece = metric_cal.compute_group_wise(
            pred_soft,
            y_true,
            region_grouper.metadata_to_group(metadata),
            region_grouper.n_groups)
        ...
        all_results_ece[f'{metric.name}_worst_year'] = all_results_ece.pop(metric_cal.worst_group_metric_field)
        region_metric_list = []; region_metric_list_ece = []
        ...
        for group_idx in range(region_grouper.n_groups):
            ...
            group_metric_ece = region_results_ece[metric_cal.group_metric_field(group_idx)]
            group_counts_ece = region_results_ece[metric_cal.group_count_field(group_idx)]

            all_results[f'{metric.name}_{group_str}'] = group_metric
            all_results[f'count_{group_str}'] = group_counts
            all_results_ece[f'{metric_cal.name}_{group_str}'] = group_metric_ece
            all_results_ece[f'count_{group_str}'] = group_counts_ece
            
            if region_results[metric.group_count_field(group_idx)] == 0 or "Other" in group_str:
                continue
            if region_results_ece[metric_cal.group_count_field(group_idx)] == 0 or "Other" in group_str:
                continue
            ...
            all_results_str_ece += (
                f'  {region_grouper.group_str(group_idx)}  '
                f"[n = {region_results_ece[metric_cal.group_count_field(group_idx)]:6.0f}]:\t"
                f"{metric_cal.name} = {region_results_ece[metric_cal.group_metric_field(group_idx)]:5.3f}\n")
            region_metric_list_ece.append(region_results_ece[metric_cal.group_metric_field(group_idx)])
        ...
        all_results_ece[f'{metric_cal.name}_worst_region'] = metric_cal.worst(region_metric_list_ece)
        all_results_str_ece += f"Worst-group {metric_cal.name}: {all_results_ece[f'{metric_cal.name}_worst_region']:.3f}\n"

        return all_results, all_results_str, all_results_ece, all_results_str_ece